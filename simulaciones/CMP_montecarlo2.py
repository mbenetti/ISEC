...
def calcular_cmp(lista_operaciones, k=K_PENALIZACION):
    ...
    # CT (Costo Total): Suma de todas las operaciones
    ct = np.sum(lista_operaciones)
...
plt.xticks(np.arange(0.1, 10.0, 1))
...
print(f"Desviación Estándar (Errores): {np.std(resultados_error):.4f}")
print(f"Media CMP (Divergencia real): {np.mean(resultados_divergencia):.4f}")
print(f"Desviación Estándar (Divergencia real): {np.std(resultados_divergencia):.4f}")
